...
0.1++	August 2019		edited for gcc by Neil Barnes
				1) massive style changes, unwrapping multiple functions on
				   the same line, removing multiple assignments, adding
				   braces and reformatting to Allman style 
				2) added -c option to reduce instruction set for cisc-8 processor
				3) compiles under linux with gcc *.c -o acisc
...
typedef enum {
	A85,			// default operation
	CISC,			// Cisc-8 reduced instructions
} TARGET;
...
TOKEN 		token;
TARGET		target = A85;				// what code will we generate?
TARGET		blob = A85;					// normally, don't produce a blob
...
int main (int argc, char ** argv)
{
    ...
    printf("added Cisc-8 target by DNB August 2019\n\n");
    ...
    while (--argc > 0) 
    {
		if (**++argv == '-') 
		{
			switch (toupper(*++*argv)) 
			{
				...
				case 'C':
					// reduced instruction set for cisc-8 processor
					printf ("Assembling for Cisc-8 target\n");
					target = CISC;
					break;
    ...
    while (++pass < 3) 
    {
		...
		while (!done) 
		{
			...
			if (newline()) 
			{  
				// reach EOF instead of "END" statement
				error('*');
...
void asm_line(void)
{
    ...
    SCRATCH int i;
    int popc (void);
    OPCODE *find_code (char *);
    OPCODE *find_code_c8 (char *);
    ...
    if ((i = popc()) != '\n') 
    {
		...
		else 
		{
			pushc(i);  
			pops(token.sval);
			// decide which processor to assemble for
			if (CISC == target)
			{
				if (!(opcod = find_code_c8(token.sval))) 
				{
					error('O');
				}
			}
