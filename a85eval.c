...
unsigned pre;
{
    ...
    for (;;) 
    {
	    ...
	    case OPR:	
			...
			u = (op == '*' ? pc : eval((op == '+' || op == '-') ? (unsigned) UOP1 : token.attr & PREC)); //HRJ from a68eval.c
			switch (op) 
			{
			    case '-':	
					u = word(0-u);  
...
int popc(void)
{
    ...
    for (;;) 
    {
		...
		if (c == '!' && !quote) 
		{ //HRJ permits !<sp> to combine lines of assembly
			eol = TRUE;  
			*lptr++ = '!'; // as in pop h! pop d
			*lptr++ = '\n'; 
